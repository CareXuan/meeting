...
17. TCP的拥塞控制
	网络拥塞就是某一资源的需求超过了可用不分，就造成了性能降低
	TCP拥塞控制的算法有四种
	1. 慢开始
	    慢开始就是刚开始发送很少，然后指数形式增长，达到阈值再增加就要用拥塞控制算法
	2. 拥塞控制
	    拥塞控制算法在请求数达到阈值之后使用的一个方法，就是把慢开始算法的指数增长编程线性增长，直到发生报文丢失且重传失败，就认为拥塞了，就会重新从1开始进行慢开始，并且对应的调低慢开始和拥塞避免的切换阈值
	3. 快重传
	    有的时候个别报文段会在网络中丢失，但是实际上并没有拥塞，但是因为报文段的丢失，接收方就无法返回确认信息，导致发送方超时重传，就会误认为拥塞发生从而直接回归慢开始，降低了传输效率。
	    快重传说的意思就是，发送方按顺序发送，并且是在收到确认之前就发送下一个请求，但接收方要判定发送方发送过来的报文是不是连续的，如果不是连续的则返回上一个确认的报文段。举例说明，发送方发送了M1,M2,M3,M4，其中M3丢失，那么接收方接收到的是M1,M2,M4，收到M1确认M1，收到M2确认M2，收到M4的时候发现M4和M2不连续，则继续返回M2，以此类推。在收到三个相同的确认信息之后，就认为当前确认的那个报文的下一段发生了丢失，则从那个报文段开始重传，而不是认为发生了阻塞
	4. 快恢复
	    在收到三个重复的确认信息之后，则认为是报文丢失而不是拥塞，启用快恢复，把慢开始的上限阈值降低成一半，继续拥塞避免算法，并且之前发送的三个未被确认的报文段会进入接收方缓存。